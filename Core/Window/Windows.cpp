...
namespace Iridescent {
	...
	{
		IRID_CORE_ERROR("GLFW Error ({0}): {1}", error, description);
	}
	...
	void Windows::Init(const WindowProps& props)
	{
		...
		IRID_CORE_INFO("Creating window {0} ({1}, {2})", props.Title, props.Width, props.Height);

		if (!s_GLFWInitialized)
		{
			// TODO: glfwTerminate on system shutdown
			int success = glfwInit();
			IRID_CORE_ASSERT(success, "Could not intialize GLFW!");
